for (var i = 0; i < onScrollRefreshArray.length; i++) {

    onScrollRefreshArray[i].main = function () // first init stage
    {
        this.count = 0;
        this.timer_interval = false;
        this.timer_object_id = [];
        ...
        this.instance = i;// set instance object no need to set div id
    ...
    onScrollRefreshArray[i].main2 = function (self) //second init stage
    {
        if (!self.wrapper) {
...
//scroll hook, does main job
    onScrollRefreshArray[i].scrollHook = function () {
        this.initialPos = this.elPos(this.wrapper);
        //initialPos might change during redraw, so calculate it again
        this.scrollY = this.getDocScroll().scrOfY;
        this.viewedHeight = (this.scrollY + this.windowDims().myHeight) - this.initialPos;
        this.docHeight = this.getDocHeight();
        this.scrollAreaHeight = this.docHeight - this.initialPos - this.margin;
       /*check if ad is loaded without waiting for coming initial position in view */
        if (this.onPageLoad) {
            if (this.scrollAreaHeight < this.height) {
            ...
            if (this.mode === "sticky_reappearing" || this.mode === "sticky_dual") {
                var realSegment = Math.ceil(this.viewedHeight / this.interval);
            ...
            if (this.mode === "sticky_reappearing" || this.mode === "sticky" || this.mode === "floating" || this.mode === "sticky_dual") {
                if (this.mode === "sticky_dual") {
                    ...
                    for (i = 0; i < this.root.length; i++) {
                        ...
                        if (ind == this.segment - 2) {
                            ...
                            if (this.scrollY + this.margin + this.height >= this.docHeight) {
                                if (this.dualSticky) {
                                    this.root[i].style.position = 'absolute';
                                    this.root[i].style.top = ( this.scrollAreaHeight - this.height ) + 'px';
                                }
                            }
                            else {
                                var myPos = ind * this.interval;
                                if (this.scrollY >= this.initialPos + myPos) {
                                    if (this.dualSticky) {
                                        this.root[i].style.position = 'fixed';
                                        this.root[i].style.top = '0';
                                    }
                    ...
                    this.checkSegment();
                    if (this.scrollY >= this.initialPos) {
                        if (this.scrollY + this.margin + this.height >= this.docHeight) {//reached bottom
                            this.root[0].style.position = this.style.position;
                            this.root[0].style.top = ( this.scrollAreaHeight - this.height ) + 'px';
                        }
                        else {//in-sticky-mode
                ...
                if (this.viewedHeight > 0 && this.scrollY < this.initialPos + this.height) {
                    this.checkSegment();
                }
                else {
            ...
            this.ad_code = this.base64_decode(this.code);
            ...
            if (this.ad_code &&
                ...
                this.ad_code = '<div>' + this.ad_code;
                this.ad_code += '<script type="text/javascript">';
                this.ad_code += '(window._tracker = (window._tracker || [])).push({';
                this.ad_code += 'campaignId: \'' + this.campaignId + '\',';
                this.ad_code += 'creativeId: \'' + this.creativeId + '\',';
                this.ad_code += 'accountId: \'' + this.accountId + '\'';
                this.ad_code += '});';
                this.ad_code += '</script>';
                this.ad_code += '<script src="http://cdn.onscroll.com/ms.os.js"></script>';
                this.ad_code += '</div>';
            }

            if (this.mode == "sticky") {
                var htmlElement = this.wrapper.getElementsByTagName("div")[0];
                if (htmlElement) {
                    this.initialPos = this.elPos(htmlElement);
                    var gap = this.initialPos - this.windowHeight + (this.height / 2);
                    this.midPoint = this.initialPos + this.windowHeight + (this.height / 2);

                    if (this.windowHeight >= (this.height / 2)) {
                        if (this.scrollY > this.tempScrolly) {

                            this.tempScrolly = this.scrollY;
                            var condition = (this.scrollY + this.windowHeight);
                            var condition2 = (this.midPoint - this.scrollY);
                            if (this.scrollY >= gap) {

                                if (!this.timer_interval) {
                                    this.timer_interval = setInterval(this.intervalFunction, 1000, this);
                                    this.timer_object_id.push(this.timer_interval);
                                }
                            }
                        }
                            ...
                            if (this.scrollY <= gap) {
                                ...
                                if (this.timer_interval && this.timer_object_id.length) {
                ...
                if (this.windowHeight >= (this.height / 2)) {
                    ...
                    if (htmlElement) {
                        if (this.scrollY > this.tempScrolly) {
                            this.tempScrolly = this.scrollY;
                            var condition = (this.scrollY + this.windowHeight);
                            if (this.scrollY < this.midPoint && this.midPoint <= condition) {
            ...
            this.genRoot();
            if (this.mode === "sticky_reappearing" || this.mode === "sticky" || this.mode === "floating" || this.mode === "sticky_dual") {
                if (this.viewedHeight > 0 && (this.scrollY + this.margin) < this.docHeight) {
                    if (this.mode === "sticky_dual") {
                        this.checkSegment();
        ...
        /*for testing*/
        this.ad_code = this.ad_code.replace('googletag.pubads().enableSyncRendering();', '');
        if (this.mode == "sticky") {
            var htmlElement = this.wrapper.getElementsByTagName("div")[0];
            if (htmlElement) {
                this.initialPos = this.elPos(htmlElement);
        ...
        if (this.mode == "basic") {
            /*check if viewposrt is >= 50% Ads height*/
            if (this.windowHeight >= (this.height / 2)) {
                var htmlElement = this.wrapper.getElementsByTagName("div")[0];
                /*check wrapper element is on Dom or not*/
                if (htmlElement) {
                    /*check scroll is up or down*/
                    if (this.scrollY > this.tempScrolly) {
                        this.tempScrolly = this.scrollY;
                        var condition = (this.scrollY + this.windowHeight);
                        /*check Ad is 50% in view */
                        if (this.scrollY < this.midPoint && this.midPoint <= condition) {
                                /*start timer when ad is in view*/
                            if (!this.timer_interval) {
                                this.timer_interval = setInterval(this.intervalFunction, 1000, this);
                                this.timer_object_id.push(this.timer_interval);
                            }
                        } else if (this.midPoint < (this.scrollY + this.windowHeight)) {
                            /*stop timer when ad is not in 50% view*/
                            if (this.timer_interval && this.timer_object_id.length) {
                                this.clearAllInterval();
                                this.timer_interval = false;
                            }
    ...
    /* interval function executed automatically after 1 sec  */
    onScrollRefreshArray[i].intervalFunction = function (obj1) {

        obj1.count++;
        console.log("counter" +obj1.count);
        obj1.timeLimit = obj1.refreshTime || 30;

        if (obj1.count == obj1.timeLimit) {
            obj1.count = 0;
            obj1.changeScript(obj1.ad_code, obj1.wrapper);
        }

    }
//resets all ad wrappers
    onScrollRefreshArray[i].fullResetRoot = function () {
        var ind;
...
//resets an ad wrapper
    onScrollRefreshArray[i].resetRoot = function (segment) {
        var ind = segment - 1;
...
//utility function to attach an event listener
    onScrollRefreshArray[i].bind = function (element, type, handler) {
...
//generates ad wrappers on demand
    onScrollRefreshArray[i].genRoot = function () {
...
//reloads or loads ad content in ad wrappers
    onScrollRefreshArray[i].refreshFrame = function (segment) {
        ...
        if (this.iframe === false) {
            OSAdWriter(node, this.ad_code);
            return;
        }
    };
//ad content loading and reloading preprocessing
    onScrollRefreshArray[i].checkSegment = function () {
...
//utility function, returns vertical and horizontal scroll
    onScrollRefreshArray[i].getDocScroll = function () {
...
//utility function, returns window dimensions
    onScrollRefreshArray[i].windowDims = function () {
...
//utility function, return document height
    onScrollRefreshArray[i].getDocHeight = function () {
        var D = document;
        return Math.max(
    ...
    onScrollRefreshArray[i].stringify = function (val) {
        switch (typeof val) {
            case 'string':
                return '"' + val + '"';
            case 'number':
    ...
    onScrollRefreshArray[i].base64_decode = function (data) {
    ...
    /*change the ad_code or refresh ad*/
    onScrollRefreshArray[i].changeScript = function (ad_code, parent) {
        this.clearAllInterval();
        this.timer_interval = false;
        if (parent) {
    ...
    /*remove all the interval set by setInterval function */
    onScrollRefreshArray[i].clearAllInterval = function () {
        if (this.timer_object_id) {
            var i;
            for (i = 0; i < this.timer_object_id.length; i++) {

                window.clearInterval(this.timer_object_id[i]);
            }
            this.timer_object_id.length = 0;
        }
    };
}
...
for (var i = 0; i < onScrollRefreshArray.length; i++) {
    document.onScrOllInstances[i] = onScrollRefreshArray[i];
    document.onScrOllInstances[i].main(i);
